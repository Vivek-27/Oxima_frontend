...
const Navbar = () => {
  ...
  const Menu = () => (
    <ul className="absolute z-40 top-16 right-0 border bg-white text-black min-w-80 rounded-2xl w-fit py-5 px-4 shadow-2xl space-y-4 transition-all duration-300">
      ...
      <li className="flex justify-between items-center bg-gray-100 px-4 py-3 rounded-lg font-medium text-gray-800">
        <span>Oxima Cash</span>
        <span className="text-green-600 font-bold">₹ {totalEarnings}</span>
      </li>

      <li className="flex items-center gap-3 px-4 py-3 line-through rounded-lg cursor-pointer text-gray-800">
  ...
  return (
    <nav className="fixed  z-10 w-full bg-black text-white flex items-center justify-between h-16 px-8 md:px-32 shadow-md">
      <div className="flex items-center gap-6">
        <Link to="/" className="text-2xl font-bold flex items-baseline gap-1">
          Oxima
        </Link>
      ...
      <div className="flex items-center gap-4">
        <div className="hidden md:flex items-center bg-white rounded-full w-64 h-9 px-4">
          ...
          <input
            ...
            className="ml-2 w-full text-sm line-through text-black placeholder-gray-600 outline-none"
          />
        </div>
