...
import locationMarker from '/public/location_marker_.png';
import customerPin from '/public/customerPin.png';
...
const Map = ({ coordinates }) => {
  ...
  const FitBounds = ({ source, destination, midpoint }) => {
    ...
    useEffect(() => {
      ...
      if (points.length === 1) {
        map.setView(points[0], 15); // just center and zoom if only one point
    ...
    <div>
      ...
      >
        ...
        <>
          ...
          {source && (
            <Marker position={source} icon={blueDotIcon}>
              <Popup>You</Popup>
            </Marker>
          )}
          {destination && (
          ...
          {midpoint && (
            <Marker position={midpoint} icon={customerIcon}>
